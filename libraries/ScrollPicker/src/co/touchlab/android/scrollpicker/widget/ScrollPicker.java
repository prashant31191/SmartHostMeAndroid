...
public class ScrollPicker extends LinearLayout {
    ...
    public interface OnValueChangeListener {
        void onValueChange(ScrollPicker picker, int oldVal, int newVal);
    }


    public interface OnCustomWeightAddedListener {
        void onCustomWeightAdded(String newVal);
    }

    public interface OnCustomRepAddedListener {
        void onCustomRepAdded(String newVal);
    }
    ...
    class PressedStateHelper implements Runnable {
        ...
        public void cancel() {
            mMode = 0;
            mManagedButton = 0;
            ScrollPicker.this.removeCallbacks(this);
            ...
            switch (mMode) {
                ...
                case MODE_TAPPED: {
                    switch (mManagedButton) {
                        case BUTTON_INCREMENT: {
                            if (!mIncrementVirtualButtonPressed) {
                                ScrollPicker.this.postDelayed(this,
    ...
    private static final int DEFAULT_LAYOUT_RESOURCE_ID = R.layout.scroll_picker;
    private static final long DEFAULT_LONG_PRESS_UPDATE_INTERVAL = 150;
    ...
    private OnCustomWeightAddedListener mOnCustomWeightAddedListener;
    private OnCustomRepAddedListener mOnCustomRepAddedListener;
    ...
    public ScrollPicker(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray attributesArray = theme.obtainStyledAttributes(attrs,
                R.styleable.ScrollPicker, defStyle, R.style.ScrollPicker);

        final int layoutResId = attributesArray.getResourceId(
                R.styleable.ScrollPicker_android_layout,
                DEFAULT_LAYOUT_RESOURCE_ID);

        mSolidColor = attributesArray.getColor(
                R.styleable.ScrollPicker_solidColor, 0);
        mHighlightColor = attributesArray.getColor(
                R.styleable.ScrollPicker_highlightColor, 0);

        mSelectionDivider = attributesArray
                .getDrawable(R.styleable.ScrollPicker_selectionDivider);

        mVirtualButtonPressedDrawable = mHighlightColor != 0 ? new ColorDrawable(mHighlightColor)
        : mSelectionDivider;
        ...
        mSelectionDividerHeight = attributesArray.getDimensionPixelSize(
                R.styleable.ScrollPicker_selectionDividerHeight,
                defSelectionDividerHeight);
        
        String fontFamily = attributesArray.getString(R.styleable.ScrollPicker_android_fontFamily);
        ...
        if (tf != null) mInputText.setTypeface(tf);
        
        mTextSize = attributesArray.getDimensionPixelSize(
                R.styleable.ScrollPicker_android_textSize,
                (int) mInputText.getTextSize());

        mMinHeight = attributesArray.getDimensionPixelSize(
                R.styleable.ScrollPicker_android_minHeight,
                mTextSize * 5);
        mMaxHeight = attributesArray.getDimensionPixelSize(
                R.styleable.ScrollPicker_android_maxHeight,
                SIZE_UNSPECIFIED);

        mMinWidth = attributesArray.getDimensionPixelSize(
                R.styleable.ScrollPicker_android_minWidth,
                mTextSize * 5);
        mMaxWidth = attributesArray.getDimensionPixelSize(
                R.styleable.ScrollPicker_android_maxWidth,
                SIZE_UNSPECIFIED);
        ...
        mInputText.setHighlightColor(mHighlightColor);
        mInputText.setBackgroundDrawable(buildHighlightDrawable());
            ...
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                if (actionId == EditorInfo.IME_ACTION_DONE) {
                    if(!TextUtils.isEmpty(mInputText.getText().toString())){
                        if(mOnCustomWeightAddedListener!=null){
                            mOnCustomWeightAddedListener.onCustomWeightAdded(mInputText.getText().toString());
                        }
                        if(mOnCustomRepAddedListener!=null){
                            mOnCustomRepAddedListener.onCustomRepAdded(mInputText.getText().toString());
                        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mVirtualButtonPressedDrawable != null
            ...
            if (mDecrementVirtualButtonPressed) {
                mVirtualButtonPressedDrawable.setState(new int[] {
        ...
        if (mHighlightColor != 0) {
            ...
            canvas.drawRect(0, topOfTopDivider, getRight(), bottomOfTopDivider, paint);
            canvas.drawRect(0, topOfBottomDivider, getRight(), bottomOfBottomDivider, paint);
        }
        else if (mSelectionDivider != null) {
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(ScrollPicker.class.getName());
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
        ...
        if (changed) {
            ...
            // Changed
            mTopSelectionDividerTop = 0 + (getHeight() / 3) - mSelectionDividerHeight;
            mBottomSelectionDividerBottom = getHeight() - (getHeight() / 3)
                    + mSelectionDividerHeight;
        }
    }
    ...
    public void setDisplayedValues(ArrayList<String> displayedValues) {
    ...
    public void setInputType(int type) {
        mInputText.setRawInputType(type);
    }
    ...
    public void setOnCustomWeightAddedListener(OnCustomWeightAddedListener onCustomWeightAddedListener ) {
        mOnCustomWeightAddedListener = onCustomWeightAddedListener;
    }

    public void setOnCustomRepAddedListener(OnCustomRepAddedListener onCustomRepAddedListener ) {
        mOnCustomRepAddedListener = onCustomRepAddedListener;
    }
    ...
    private void showSoftInput() {
        InputMethodManager inputMethodManager = (InputMethodManager) getContext()
                .getSystemService(Context.INPUT_METHOD_SERVICE);
        if (inputMethodManager != null&&mShowEditText) {
    ...
    private void validateInputTextView(EditText v) {
        String str = String.valueOf(v.getText());
        if (TextUtils.isEmpty(str)) {
            updateInputTextView();
        } else {
            if(mOnCustomRepAddedListener!=null){
                mOnCustomRepAddedListener.onCustomRepAdded(str);
            }
            if(mOnCustomWeightAddedListener!=null){
                mOnCustomWeightAddedListener.onCustomWeightAdded(str);
            }
    ...
    /*
    Its setting the internal value based on the index of the newly added value
    then when we save the set we see that the index isn't part of the display values so we add it
     */
    public void setValueInternal(String val){
        setValueInternal(getSelectedPos(val), true);
    }
}
